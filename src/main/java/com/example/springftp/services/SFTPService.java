...
public class SFTPService {
    ...
    private final String DIR_PATH = "uploads/";
    ...
    public MessageHandler fileExistanceChecker() {
        return message -> {
            ...
            if (!requestedFile.exists(message.getPayload().toString()))
                throw new FileNotFoundException(format("Requested file [%s] not found!", message.getPayload().toString()));
        };
    }

    @Bean
    @ServiceActivator(inputChannel = "fromSftpChannel")
    public MessageHandler downloadFile() {
